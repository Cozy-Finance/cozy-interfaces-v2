...
interface ICozyLens {
  ...
  function isSet(ISet _set) external view returns (bool _exists);

  /// @notice Returns true if the provided `_set` is approved to pull funds from the Backstop.
  function isSetApprovedForBackstop(ISet _set) external view returns (bool _approved);

  /// @notice Returns the set configUpdateTime
  function getConfigUpdateTime(ISet _set) external view returns (uint256 _configUpdateTime);

  /// @notice Returns the sets configUpdateDeadline
  function getConfigUpdateDeadline(ISet _set) external view returns (uint256 _configUpdateDeadline);
  ...
  function getActiveProtection(ISet _set, address _trigger) external view returns (uint256 _amount);

  /// @notice Returns the last decay time of the market, identified by its `_set` and `_trigger` address.
  function getLastDecayTime(ISet _set, address _trigger) external view returns (uint256 _timestamp);

  /// @notice Returns the current decay rate of the market, identified by its `_set` and `_trigger` address, as a wad.
  function getDecayRate(ISet _set, address _trigger) external view returns (uint256 _decayRate);

  /// @notice Returns the purchase fee of the market, identified by its `_set` and `_trigger` address, as a zoc.
  function getMarketPurchaseFee(ISet _set, address _trigger) external view returns (uint256 _purchaseFee);

  /// @notice Returns the weight of the market, identified by its `_set` and `_trigger` address, as a zoc.
  function getMarketWeight(ISet _set, address _trigger) external view returns (uint256 _weight);

  /// @notice Returns the address of the market's cost model, identified by its `_set` and `_trigger` address.
  function getMarketCostModel(ISet _set, address _trigger) external view returns (ICostModel _costModel);
  ...
  function remainingWithdrawalDelay(ISet _set, uint256 _withdrawalId) external view returns (uint256 _delay);

  /// @notice Returns the `SetConfig` for the `_set`.
  function getSetConfig(ISet _set) external view returns (SetConfig memory);

  /// @notice Returns the `MarketInfo` for the market, identified by its `_set` and `_trigger` address.
  function getMarketInfo(ISet _set, address _trigger) external view returns (MarketInfo memory);

  /// @notice Returns an array of all trigger addresses in the `_set`.
  function allTriggers(ISet _set) external view returns (address[] memory _triggers);
}
