...
interface ICozyLens {
  ...
  function isSet(address _set) view external returns (bool _exists);

  /// @notice Returns true if the provided `_set` is approved to pull funds from the Backstop.
  function isSetApprovedForBackstop(address _set) view external returns (bool _approved);

  /// @notice Returns the set configUpdateTime
  function getConfigUpdateTime(address _set) view external returns (uint256 _configUpdateTime);

  /// @notice Returns the sets configUpdateDeadline
  function getConfigUpdateDeadline(address _set) view external returns (uint256 _configUpdateDeadline);
  ...
  function getMarketState(address _set, address _trigger) view external returns (uint8 _state);

  /// @notice Returns the state of the `_set`.
  function getSetState(address _set) view external returns (uint8 _state);

  /// @notice Returns the PToken address of the market, identified by its `_set` and `_trigger` address.
  function getPToken(address _set, address _trigger) view external returns (address _ptoken);
  ...
  function getActiveProtection(address _set, address _trigger) view external returns (uint256 _amount);

  /// @notice Returns the last decay time of the market, identified by its `_set` and `_trigger` address.
  function getLastDecayTime(address _set, address _trigger) view external returns (uint256 _timestamp);

  /// @notice Returns the current decay rate of the market, identified by its `_set` and `_trigger` address, as a wad.
  function getDecayRate(address _set, address _trigger) view external returns (uint256 _decayRate);

  /// @notice Returns the purchase fee of the market, identified by its `_set` and `_trigger` address, as a zoc.
  function getMarketPurchaseFee(address _set, address _trigger) view external returns (uint256 _purchaseFee);

  /// @notice Returns the weight of the market, identified by its `_set` and `_trigger` address, as a zoc.
  function getMarketWeight(address _set, address _trigger) view external returns (uint256 _weight);

  /// @notice Returns the address of the market's cost model, identified by its `_set` and `_trigger` address.
  function getMarketCostModel(address _set, address _trigger) view external returns (address _costModel);
  ...
  function getPreviewPurchaseDataCost(address _set, address _trigger, uint256 _protection) view external returns (uint256 _cost);
  ...
  function remainingWithdrawalDelay(address _set, uint256 _withdrawalId) view external returns (uint256 _delay);

  /// @notice Returns the `SetConfig` for the `_set`.
  function getSetConfig(address _set) view external returns (SetConfig memory);

  /// @notice Returns the `MarketInfo` for the market, identified by its `_set` and `_trigger` address.
  function getMarketInfo(address _set, address _trigger) view external returns (MarketInfo memory);

  /// @notice Returns an array of all trigger addresses in the `_set`.
  function allTriggers(address _set) view external returns (address[] memory _triggers);
  ...
  function computeSetAddress(bytes32 _salt, address _asset, SetConfig memory _setConfig, SetConfig[] memory _marketInfos) view external returns (address);

  /// @notice Given a `_set` and its `_trigger` address, compute and return the address of its PToken.
  function computePTokenAddress(address _set, address _trigger) view external returns (address);
}
